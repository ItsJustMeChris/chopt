...
public final class TreeChopper {
	...
	// MC 26.2 dropped these BlockTags constants, but still ships the data tags themselves
	// (data/minecraft/tags/block/*.json), so referencing them by id keeps the old behavior.
	private static final TagKey<Block> CRIMSON_STEMS = vanillaBlockTag("crimson_stems");
	private static final TagKey<Block> WARPED_STEMS = vanillaBlockTag("warped_stems");
			...
			for (Block block : BuiltInRegistries.BLOCK) {
				...
				for (BlockTransformer.BlockTransformData data : axe.transforms()) {
					...
					try {
						...
						// ProbeLevel only answers getBlockState; if a rule reaches for anything
						// else we skip that entry rather than take the chop path down with us.
						continue;
					}
					if (stripped != null && stripped.getBlock() != block) {
						table.put(block, stripped.getBlock());
						break;
					}
				}
			}
	...
	/**
	 * Minimal stand-in level used to evaluate vanilla's strip rules outside the world.
	 * Only {@code getBlockState} is ever consulted; everything else answers with a
	 * harmless default.
	 */
	private static final class ProbeLevel implements InvocationHandler {
		BlockState state = Blocks.AIR.defaultBlockState();

		LevelAccessor asLevelAccessor() {
		...
		public Object invoke(Object proxy, Method method, Object[] args) {
			...
			if ("toString".equals(name)) return "ProbeLevel";
	...
	private static boolean hasLeavesNearby(Level level, Map<BlockPos, BlockState> originals) {
		for (Map.Entry<BlockPos, BlockState> entry : originals.entrySet()) {
			...
			boolean isNetherStem = entry.getValue().is(CRIMSON_STEMS) || entry.getValue().is(WARPED_STEMS);
