...
public enum ShrinkingStumpProvider implements IBlockComponentProvider {
	...
	public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
		...
		if (!data.contains(ShrinkingStumpData.KEY_REQUIRED)) {
			return;
		}
		int required = data.getInt(ShrinkingStumpData.KEY_REQUIRED);
		if (required <= 0) {
			return;
		}
		int hits = Math.min(data.getInt(ShrinkingStumpData.KEY_HITS), required);
